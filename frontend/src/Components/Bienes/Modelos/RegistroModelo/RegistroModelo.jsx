...
function RegistroModelo() {
    ...
    const handleCrearModelo = async (data) => {
        modelosRequests.registrarModelo(data).then(response => {
            if (!response.error) {
                formModelo.reset();
                toast.current.show({severity: 'success', summary: 'Registro de Modelo de Bien', detail: 'Registro exitoso.', life: 3000});
                navigate(-1);
            } else {
    ...
    return (
        <div className='grid col-11 md:col-9 mx-auto p-4 p-fluid bg-gray-50 border-round shadow-1 mb-4 md:row-gap-3'>
            ...
            <div className='col-12 flex flex-wrap justify-content-between gap-2 md:mt-4'>
                ...
                <div className='col-12 md:col p-0'>
                    <Button
                        severity='info' label='Registrar Modelo' icon='pi pi-plus'
                        className='w-full'
                        onClick={handleSubmit(handleCrearModelo)}
                    />
                </div>
            </div>
        </div>
